...
void APS6404_write_byte(uint8_t byte) {
	OSPI_RegularCmdTypeDef sCommand = {0};
	...
	/* Transmit data to the APS6404L */
	if (HAL_OSPI_Transmit(&hospi1, &byte, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
	  Error_Handler();
	}
}
...
uint8_t APS6404_read_byte() {
	...
	/* Transmit data to the APS6404L */
	if (HAL_OSPI_Receive(&hospi1, &byte, HAL_OSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
	  Error_Handler();
	}
